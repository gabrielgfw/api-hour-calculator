$(document).ready(() => {
  ...
  */
  ...
  $("#btnCalcular").click(() => {
    ...
    function calcularHoras(infoHoras) {
      try {
        const { startHour, finalHour } = infoHoras;
        ...
        if (!ehUmaDataValida(startHour, finalHour) || !isValidResult(resultCalcInMs)) {
          throw new Error({
            message: "Error: Dados inválidos para cálculo."
          });
        }

        const objetoFinal = formatarObjetoFinalCalculo(resultObject);
        registrarResultado(arquivoResultados, objetoFinal);
        ...
        chamarPopUp("falhou", error.message);
      }
    }

    function ehUmaDataValida(start, final) {
      ...
      if (!(moment(startDate).isValid()) || !(moment(finalDate).isValid())) {
        return false;
      }
    ...
    function formatarObjetoFinalCalculo(resultObj) {
    ...
    function mostrarResultado() {
      // Limpar Elementos (?)
      // Limpa todos os elementos referete a resultados já exibidos,
